...
return(
  <div className="main-grid">
            ...
            image.map((curr ,index)=>{
              return(
                <figure key={index}  className='figure-product'>
                  ...
                  <img src={resolveAssetUrl(curr)}  className='box-image-style' alt={curr.filename} key={index} onClick={()=> setMainImage(curr)} />
                    </div>
                </figure>
              )
            })
    ...
    <div className="main-screen">
      <img src={resolveAssetUrl(mainImage)} alt="images" />
    </div>
  </div>
)
}
